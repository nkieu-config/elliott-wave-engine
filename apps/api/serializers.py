"""Pipeline → JSON serializers shared by the API and the offline export, so the
static snapshot and live API return identical shapes."""
...
def serialize_pivot(p: Pivot) -> dict[str, Any]:
    return {
        "index": p.index,
...
def serialize_segment(s: Segment) -> dict[str, Any]:
    return {"start": serialize_pivot(s.start), "end": serialize_pivot(s.end)}


def _serialize_link_set(s: Any) -> dict[str, Any]:
...
def serialize_wave(node: WaveNode) -> dict[str, Any]:
    return {
        ...
        "sets": (
            [_serialize_link_set(s) for s in node.sets] if node.sets else None
        ),
    }


def serialize_scenario(s: Scenario) -> dict[str, Any]:
...
def serialize_diagnostic(d: Any) -> dict[str, Any]:
...
def serialize_report(r: AnalysisReport | None) -> dict[str, Any] | None:
    if r is None:
        return None
    return {
...
def serialize_pipeline(
    ...
    *,
    meta: dict[str, Any],
    top_scenario_layer1: dict[str, Any] | None = None,
) -> dict[str, Any]:
...
def serialize_theory_ref(ref: TheoryRef) -> dict[str, Any]:
    return {
...
def serialize_target(t: Target) -> dict[str, Any]:
    return {
        "name": t.name,
        "price": t.price,
        "type": t.type,
...
def serialize_target_set(ts: TargetSet | None) -> dict[str, Any] | None:
    if ts is None:
        return None
    return {
...
def serialize_bottleneck(b: BottleneckDiagnosis | None) -> dict[str, Any] | None:
    if b is None:
        return None
    return {
...
def serialize_confirmation_level(lv: ConfirmationLevel) -> dict[str, Any]:
    return {
        "name": lv.name,
...
def serialize_confirmation(c: ConfirmationReport | None) -> dict[str, Any] | None:
    if c is None:
        return None
    return {
        "family": c.family,
...
def serialize_price_move(m: PriceMove | None) -> dict[str, Any] | None:
    if m is None:
        return None
    return {
        "label": m.label,
...
def serialize_decision(d: DecisionSummary | None) -> dict[str, Any] | None:
    if d is None:
        return None
    return {
...
def serialize_alternative(a: AlternativeBrief | None) -> dict[str, Any] | None:
    if a is None:
        return None
    return {
        "family": a.family,
...
def serialize_next_pattern(n: NextPattern) -> dict[str, Any]:
    return {
...
def serialize_succession(s: SuccessionReport | None) -> dict[str, Any] | None:
    if s is None:
        return None
    return {
        "family": s.family,
...
def serialize_analysis_result(r: AnalysisResult) -> dict[str, Any]:
    return {
